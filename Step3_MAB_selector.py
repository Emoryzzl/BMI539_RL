# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 00:31:03 2024
@author: zzha962
"""
...
from contextualbandits.online import LinUCB, AdaptiveGreedy, \
        SoftmaxExplorer, EpsilonGreedy

# Step1: Reading the data 
X_df = pd.read_csv('Image_features.csv')  
y_df = pd.read_csv('DICE_class.csv') 

X = np.float16(X_df.iloc[:, 1:].values)
y = np.uint8(y_df.iloc[:, 1:].values)
print(X.shape)
print(y.shape)
    ...
    for i in range(int(np.ceil(X.shape[0] / batch_size))):
        ...
        for model in range(len(models)):
            lst_actions[model] = simulate_rounds_stoch(models[model],
                                                       ...
                                                       X_batch, y_batch,
                                                       rnd_seed = batch_st)
        print('{}/{} {}/{} done'.format(i+1,int(np.ceil(X.shape[0] / batch_size)), jj+1, Epoch_num))
...
plt.xlabel('Rounds (models were updated every {} rounds)'.format(batch_size), size=15)
plt.ylabel('Cumulative Mean Reward', size=15)
plt.title('Comparison of Online Contextual Bandit Policies (Streaming-data mode) \n Prostate MRI Dataset ({} categories, {} attributes)'.format(y.shape[1], X.shape[1]),size=15)
plt.grid()
plt.show()
